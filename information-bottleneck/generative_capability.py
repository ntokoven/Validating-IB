...
def train_encoder(dnn_hidden_units, dnn_input_units=784, dnn_output_units=10, z_dim = 6, enc_type='MLP', layer='Linear3', weight_decay=0, num_epochs=10, eval_freq=1, beta=1):
...
def main():
    ...
    Encoder = train_encoder(enc_type=enc_type, num_epochs=num_epochs, weight_decay=weight_decay)
    ...
    for i in range(len(seeds)):
        ...
        for layer in layers_names:
            for num_labels in num_labels_range:
                print('\n\nEvaluating for %s (labels per class - %s)' % (layer, num_labels/num_classes))
                if enc_type == 'MLP':
    ...
    acc_df.to_csv(FLAGS.result_path+'/acc_%s_%s.csv' % (enc_type.lower(), int(1/weight_decay)), sep=' ')


def plot_acc_numlabels(acc_df, layers_names, num_labels_range):
    ...
    for i in range(len(layers_names)):
        ...
        metric_value = abc_metric(Encoder.best_performance, acc_df[layers_names[i]].apply(mean).to_numpy(), nums)
        ax[i].plot(np.log10(nums), means, color=colors[i], label=layers_names[i]+' + weight_decay (%s) => ABC=%0.5f' % (weight_decay, metric_value))
        ax[i].plot(np.log10(nums), Encoder.best_performance * np.ones(len(nums)))
        ax[i].fill_between(np.log10(nums), means - stds, means + stds, facecolor=colors[i], interpolate=True, alpha=0.25)
        ax[i].grid()
        ax[i].set_ylim((np.min(acc_df.min().min()), 1))

    ax[-1].set_xlabel('# Labels per class (log10)')
    ...
    if not os.path.exists(FLAGS.result_path):
          os.makedirs(FLAGS.result_path)
    fig1.savefig(FLAGS.result_path+'/acc_%s_%s.png' % (enc_type.lower(), int(1/weight_decay)))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--default_seed', type = int, default = 1234,
